module.exports = function renderVagas(vagas = []) {
    ...
    const renderVagasCards = () => {
        ...
        return vagas.map(v => {
            ...
            // Botão de Compartilhamento Nativo MAIOR movido para o lado oposto do título
            const shareButtonHTML = `
                <button onclick="compartilharVaga('${v.titulo.replace(/'/g, "\\'")}', '${v.salario.replace(/'/g, "\\'")}')" class="shrink-0 bg-gray-100/80 text-gray-700 hover:bg-brand hover:text-white w-12 h-12 md:w-14 md:h-14 rounded-full shadow-sm z-10 flex items-center justify-center transition-all hover:scale-110 border border-gray-200" title="Compartilhar Vaga" aria-label="Compartilhar Vaga">
            ...
            <div class="bg-white rounded-lg md:rounded-xl shadow-sm border-2 border-gray-200 overflow-hidden ${isDisponivel ? 'hover:border-brand/50 hover:shadow-xl' : 'opacity-80'} transition-all duration-300 flex flex-col relative group">
                ...
                <div class="p-5 md:p-6 flex flex-col flex-grow">
                    ...
                    <div class="flex justify-between items-start gap-4 mb-4 md:mb-6">
                        <h3 class="text-xl md:text-2xl font-black text-gray-900">${v.titulo}</h3>
                        ${shareButtonHTML}
                    </div>
    ...
    <body class="text-gray-800 antialiased selection:bg-brand selection:text-white flex flex-col min-h-screen">
                    ...
                    <form action="/vagas/candidatar" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-2 gap-3 md:gap-4 mb-4">
                            <div>
                                ...
                                <input type="text" name="contato" class="w-full bg-gray-50 border border-gray-300 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-brand transition-all text-sm" required placeholder="(71) 90000-0000" oninput="mascaraTelefone(this)" maxlength="15">
                            </div>
                            <div>
        ...
        <script>
            // Nova função: Máscara Automática para o WhatsApp
            function mascaraTelefone(input) {
                let v = input.value.replace(/\\D/g, ''); // Remove tudo o que não for número
                if (v.length > 11) v = v.substring(0, 11); // Limita a 11 números (padrão Brasil celular)
